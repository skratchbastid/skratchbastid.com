<script setup>
    ...
    const futureShowListings = computed(() => {
        return shows.value.filter(show => $dayjs().isSameOrBefore($dayjs(show.eventsFields.eventDate, 'day')) || $dayjs(show.eventsFields.eventDate).isToday())
    })
</script>
    ...
    <NuxtLink to="/shows">
        <div v-if="futureShowListings.length > 1" class="relative flex py-4 md:py-6 overflow-x-hidden text-xl md:text-4xl bg-slate-900 md:rotate-[-0.0deg]"  @mouseover="paused = true" @mouseleave="paused = false">
            <div class="text-white whitespace-nowrap animate-marquee flex" :style="paused ? 'animation-play-state: paused;' :''">
                <div v-for="show in futureShowListings"><span class="mx-10 font-black uppercase">{{show.eventsFields.location}} <span class="font-thin">{{ $dayjs(show.eventsFields.eventDate).format('MMM D, YYYY')}}</span></span></div>
            </div>
